...
<html lang="en">
    <head>
        <title>Danh sách chi tiết sản phẩm | Quản trị Admin</title>
        <meta charset="utf-8">
    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <aside class="app-sidebar">
            ...
            <li><a class="app-menu__item active" href=<%=fromServlet==null?"product-detail-manage.jsp":"admin/doc/product-detail-manage.jsp"%>><i
                    ...
                    <div class="tile">
                        <div class="tile-body">
                            <div class="row element-button">
                                <div class="col-sm-2">
                                    <a class="btn btn-add btn-sm" href="/Project/ProductDetailServlet?service=sendRequestToInsert" title="Thêm"><i class="fas fa-plus"></i>
                                        Thêm mới chi tiết sản phẩm</a>
                                </div>
                                <div class="col-sm-2">
                            ...
                            <div style="
                                ...
                                <div class="search_bar" style="margin-left: 8px">
                                    <form action="/Project/ProductDetailServlet">
                                        <input style="border-radius: 5px; border-color: #F0F0F0" placeholder="Search..." type="text" name="searchProductDetail">
                                        <button style="border-radius: 30px" type="submit"><i class="fa fa-search"></i></button>
                                        <input type="hidden" name="service" value="searchProductDetail">
                            ...
                            <div>                          
                            <%
                                String msDeleteProductDetail = (String)request.getAttribute("msDeleteProductDetail");
                                if(msDeleteProductDetail != null){
                            %>
                            <script>
                                alert("<%=msDeleteProductDetail%>")
                            </script>
                            <%}%>
                        </div>
                            <table style="margin-top: 20px" class="table table-hover table-bordered">
                                ...
                                <tbody>
                                    <%
                                    Vector<ProductDetail> listProductDetail = new Vector<>();
                                    
                                    if((Vector<ProductDetail>)request.getAttribute("listProductDetail") != null){
                                        listProductDetail = (Vector<ProductDetail>)request.getAttribute("listProductDetail");
                                    }else {
                                        listProductDetail = pdd.getAllProductDetailFromSQL("select * from ProductDetail");
                                    }
                                    
                                    int checkProductDetail = 0;
                                    for(ProductDetail proD : listProductDetail){
                                        ++checkProductDetail;
                                    %>
                                    <tr>
                                        <td id="product_code<%=checkProductDetail%>"><%=proD.getProductCode()%></td>
                                        <%
                                            Product product = pd.getAllProductFromSQL("select * from Product where ProductCode = " + proD.getProductCode()).firstElement();                                           
                                        %>
                                        <td><img src="<%="/Project/" + product.getPicture()%>" alt="" width="100px;"></td>
                                        <td><%=product.getProductName()%></td>                                     
                                        <td><%=proD.getQuantity()%></td>
                                        <td id="size<%=checkProductDetail%>"><%=proD.getSize()%></td>
                                        <td id="color<%=checkProductDetail%>"><%=proD.getColor()%></td>
                                        <td>
                                            <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                    onclick="confirmDelete(<%=checkProductDetail%>)"><i class="fas fa-trash-alt"></i> 
                                            </button>
                                            <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp" data-toggle="modal"
                                                    onclick="requestUpdate('<%=checkProductDetail%>')"><i class="fas fa-edit"></i>
                                            </button>
                                        </td>
                                    </tr>
                                    <%}%>
        ...
        <script>
            function confirmDelete(checkProductDetail) {
                jQuery(function () {
                    jQuery(".trash").append(function () {
                        swal({
                            title: "Cảnh báo",
                            text: "Bạn có chắc chắn là muốn xóa chi tiết sản phẩm này không?",
                            buttons: ["Hủy bỏ", "Đồng ý"]
                        })
                                .then((willDelete) => {
                                    if (willDelete) {
                                        var productCode = document.getElementById('product_code' + checkProductDetail).textContent;
                                        var size = document.getElementById('size' + checkProductDetail).textContent;
                                        var color = document.getElementById('color' + checkProductDetail).textContent;
                                        window.location.href = '/Project/ProductDetailServlet?service=delete&productCode=' + productCode + '&size=' + size + '&color=' + color;
                                    }
                                });
                    });
                });
        ...
        <script>
            function requestUpdate(checkProductDetail) {
                var productCode = document.getElementById('product_code' + checkProductDetail).textContent;
                var size = document.getElementById('size' + checkProductDetail).textContent;
                var color = document.getElementById('color' + checkProductDetail).textContent;
                window.location.href = '/Project/ProductDetailServlet?service=sendUpdateProductDetail&status=admin&productCode=' + productCode + '&size=' + size + '&color=' + color;
            }
        </script>
    </body>
