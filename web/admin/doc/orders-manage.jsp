...
<%@page import="java.util.Vector, jakarta.servlet.http.HttpSession, model.Customer, dal.CustomerDAO, model.ProductCart, dal.ProductCartDAO, model.Product, dal.ProductDAO, model.Brand, dal.BrandDAO, java.text.DecimalFormat, model.Orders, dal.OrdersDAO, model.OrderDetails, dal.OrderDetailsDAO"%>
<!DOCTYPE html>
<html lang="en">
    <head>
                    ...
                    <div class="tile-body">
                        ...
                        <div style="
                            ...
                            <div class="search_bar" style="margin-left: 8px">
                                <form action="/Project/OrderServlet">
                        ...
                        <div>                          
                            <%
                                String msDeleteOrder = (String)request.getAttribute("msDeleteOrder");
                                if(msDeleteOrder != null){
                            %>
                            <script>
                                alert("<%=msDeleteOrder%>")
                            </script>
                        ...
                        <table style="margin-top: 20px" class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0">
                            ...
                            <tbody>
                                <%
                                    ...
                                    if((Vector<Orders>)request.getAttribute("listOrder") != null){
                                        listOrder = (Vector<Orders>)request.getAttribute("listOrder");
                                    }else {
                                        listOrder = od.getAllOrdersFromSQL("select * from Orders");
                                    }
                                ...
                                <tr>
                                    ...
                                    <td><%=order.getShipperID()%></td>
                                    <td><%=order.getOrderDate()%></td>
                                    <td><%=order.getShippedDate()%></td>
                                    <td>$<%=order.getShippingTotal()%></td>
                                    ...
                                    <td class="table-td-center">
                                        <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                onclick="confirmDelete(<%=order.getOrderID()%>)"><i class="fas fa-trash-alt"></i> 
                                        </button>
                                        <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp" data-toggle="modal"
                                                onclick="requestUpdate(<%=order.getOrderID()%>)"><i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
                                <%}%>
        ...
        function confirmDelete(orderID) {
            jQuery(function () {
                jQuery(".trash").append(function () {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn xóa đơn hàng này không?",
                        buttons: ["Hủy bỏ", "Đồng ý"]
                    })
                            .then((willDelete) => {
                                if (willDelete) {
                                    window.location.href = '/Project/OrderServlet?service=delete&orderID=' + orderID;
                                }
                            });
                });
            });
    ...
    <script>
        function requestUpdate(orderID) {
            window.location.href = '/Project/OrderServlet?service=sendUpdateOrder&status=admin&orderID=' + orderID;
        }
    </script>
</body>
