...
<%@page import="java.util.Vector, jakarta.servlet.http.HttpSession, model.Customer, dal.CustomerDAO, model.Product, dal.ProductDAO, java.text.DecimalFormat, dal.OrderDetailsDAO, model.OrderDetails"%>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <ul class="app-menu">
            ...
            <li><a class="app-menu__item active" href=<%=fromServlet==null?"order-detail-manage.jsp":"admin/doc/order-detail-manage.jsp"%>><i class='app-menu__icon bx bx-task'></i><span
                    ...
                    <div class="tile-body">
                        ...
                        <div style="
                            ...
                            <div class="search_bar" style="margin-left: 8px">
                                <form action="/Project/OrderDertailsServlet">
                                    <input style="border-radius: 5px; border-color: #F0F0F0" placeholder="Search..." type="text" name="searchOD">
                                    <button style="border-radius: 30px" type="submit"><i class="fa fa-search"></i></button>
                                    <input type="hidden" name="service" value="searchOD">
                                </form>
                            </div>
                        </div>
                        <div>                          
                            <%
                                String msDeleteOrderDetail = (String)request.getAttribute("msDeleteOrderDetail");
                                if(msDeleteOrderDetail != null){
                            %>
                            <script>
                                alert("<%=msDeleteOrderDetail%>")
                            </script>
                            <%}%>
                        </div>
                        <table style="margin-top: 20px" class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0">
                            <thead>
                                <tr>
                                    ...
                                    <th width="200">Kích thước đặt hàng</th> 
                                    <th width="200">Màu đặt hàng</th> 
                                    <th width="200">Số lượng đặt hàng</th> 
                                    <th>Tính năng</th>
                                </tr>
                            </thead>
                            <tbody>
                                <%
                                    ...
                                    if((Vector<OrderDetails>)request.getAttribute("listOD") != null){
                                        listOD = (Vector<OrderDetails>)request.getAttribute("listOD");
                                    }else {
                                        listOD = odd.getAllOrderDetailsFromSQL("select * from OrderDetails");
                                    }
                                ...
                                <tr>
                                    <td><%=od.getOrderID()%></td>
                                    <td><%=od.getProductCode()%></td>
                                    <td><%=od.getSizeOrder()%></td>
                                    <td><%=od.getColorOrder()%></td>
                                    <td><%=od.getQuantityOrder()%></td>
                                    <td class="table-td-center">
                                        <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                onclick="confirmDelete(<%=od.getOrderID()%>, <%=od.getProductCode()%>)"><i class="fas fa-trash-alt"></i> 
                                        </button>
                                        <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp" data-toggle="modal"
                                                onclick='requestUpdate("")'><i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
                                <%}%>
    ...
    <script>
        function confirmDelete(orderID, productCode) {
            jQuery(function () {
                jQuery(".trash").append(function () {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn xóa chi tiết đơn hàng này không?",
                        buttons: ["Hủy bỏ", "Đồng ý"]
                    })
                            .then((willDelete) => {
                                if (willDelete) {
                                    window.location.href = '/Project/OrderDertailsServlet?service=delete&orderID=' + orderID + '&productCode=' + productCode;
                                }
                            });
                });
            });
