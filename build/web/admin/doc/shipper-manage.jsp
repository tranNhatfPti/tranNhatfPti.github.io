...
<%@page import="java.util.Vector, jakarta.servlet.http.HttpSession, model.Customer, dal.CustomerDAO, model.ProductCart, dal.ProductCartDAO, model.Product, dal.ProductDAO, model.Brand, dal.BrandDAO, java.text.DecimalFormat, dal.ShipperDAO, model.Shipper"%>
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>Danh sách đơn vị vận chuyển| Quản trị Admin</title>
        <meta charset="utf-8">
    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <ul class="app-menu">
            ...
            <li><a class="app-menu__item active" href=<%=fromServlet==null?"shipper-manage.jsp":"admin/doc/shipper-manage.jsp"%>><i
    ...
    <main class="app-content">
        <div class="app-title">
            <ul class="app-breadcrumb breadcrumb side">
                <li class="breadcrumb-item active"><a href="#"><b>Danh sách đơn vị vận chuyển</b></a></li>
            </ul>
            <div id="clock"></div>
        </div>
        <div class="row">
                ...
                <div class="tile">
                    <div class="tile-body">
                        <div class="row element-button">
                            <div class="col-sm-2">
                                <a class="btn btn-add btn-sm" href=<%=fromServlet==null?"form-insert-shipper.jsp":"admin/doc/form-insert-shipper.jsp"%> title="Thêm"><i class="fas fa-plus"></i>
                                    Thêm mới đơn vị vận chuyển</a>
                            </div>
                        ...
                        <div style="
                            ...
                            <div class="search_bar" style="margin-left: 8px">
                                <form action="/Project/ShipperServlet">
                                    <input style="border-radius: 5px; border-color: #F0F0F0" placeholder="Search..." type="text" name="searchShipper">
                                    <button style="border-radius: 30px" type="submit"><i class="fa fa-search"></i></button>
                                    <input type="hidden" name="service" value="searchShipper">
                                </form>
                            </div>
                        </div>
                        <div>                          
                            <%
                                String msDeleteShipper = (String)request.getAttribute("msDeleteShipper");
                                if(msDeleteShipper != null){
                            %>
                            <script>
                                alert("<%=msDeleteShipper%>")
                            </script>
                            <%}%>
                        </div>
                        <table style="margin-top: 20px" class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0">
                            <thead>
                                <tr>
                                    <th width="200">Mã đơn vị vận chuyển</th>
                            ...
                            <tbody>
                                <%
                                    Vector<Shipper> listShipper = new Vector<>();
                                    
                                    if((Vector<Shipper>)request.getAttribute("listShipper") != null){
                                        listShipper = (Vector<Shipper>)request.getAttribute("listShipper");
                                    }else {
                                        listShipper = sd.getAllShipperFromSQL("select * from Shipper");
                                    }
                                ...
                                <tr>
                                    ...
                                    <td><%=shipper.getCountry()%></td>
                                    <td class="table-td-center">
                                        <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                onclick="confirmDelete(<%=shipper.getShipperID()%>)"><i class="fas fa-trash-alt"></i> 
                                        </button>
                                        <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp" data-toggle="modal"
                                                onclick='requestUpdate("<%=shipper.getShipperID()%>")'><i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
                                <%}%>
        ...
        function confirmDelete(shipperID) {
            jQuery(function () {
                jQuery(".trash").append(function () {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn xóa đơn vị vận chuyển này không?",
                        buttons: ["Hủy bỏ", "Đồng ý"]
                    })
                            .then((willDelete) => {
                                if (willDelete) {
                                    window.location.href = '/Project/ShipperServlet?service=delete&shipperID=' + shipperID;
                                }
                            });
                });
            });
    ...
    <script>
        function requestUpdate(shipperID) {
            window.location.href = '/Project/ShipperServlet?service=sendUpdateShipper&status=admin&shipperID=' + shipperID;
        }
    </script>
</body>
