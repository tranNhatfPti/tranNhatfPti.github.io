...
<%@page import="java.util.Vector, jakarta.servlet.http.HttpSession, model.Customer, dal.CustomerDAO, model.ProductCart, dal.ProductCartDAO, model.Product, dal.ProductDAO, model.Brand, dal.BrandDAO, java.text.DecimalFormat, dal.CategoryDAO, model.Category"%>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <ul class="app-menu">
            ...
            <li><a class="app-menu__item active" href=<%=fromServlet==null?"category-manage.jsp":"admin/doc/category-manage.jsp"%>><i
                ...
                <div class="tile">
                    <div class="tile-body">
                        <div class="row element-button">
                            <div class="col-sm-2">
                                <a class="btn btn-add btn-sm" href=<%=fromServlet==null?"form-insert-category.jsp":"admin/doc/form-insert-category.jsp"%> title="Thêm"><i class="fas fa-plus"></i>
                                    Thêm mới danh mục</a>
                            </div>
                        ...
                        <div style="
                            ...
                            <div class="search_bar" style="margin-left: 8px">
                                <form action="/Project/CategoryServlet">
                                    <input style="border-radius: 5px; border-color: #F0F0F0" placeholder="Search..." type="text" name="searchCategory">
                        ...
                        <div>                          
                            <%
                                String msDeleteCategory = (String)request.getAttribute("msDeleteCategory");
                                if(msDeleteCategory != null){
                            %>
                            <script>
                                alert("<%=msDeleteCategory%>")
                            </script>
                            <%}%>
                        </div>
                        <table style="margin-top: 20px" class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0">
                            <thead>
                                <tr>
                                    ...
                                    <th width="1100">Miêu tả danh mục</th>
                                    <th>Tính năng</th>
                                </tr>
                            </thead>
                            <tbody>
                                <%
                                    ...
                                    if((Vector<Category>)request.getAttribute("listCategory") != null){
                                        listCategory = (Vector<Category>)request.getAttribute("listCategory");
                                    }else {
                                        listCategory = cated.getAllCategoryFromSQL("select * from Category");
                                    }
                                ...
                                <tr>
                                    ...
                                    <td class="table-td-center">
                                        <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                onclick="confirmDelete('<%=c.getCategoryID()%>')"><i class="fas fa-trash-alt"></i> 
                                        </button>
                                        <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp" data-toggle="modal"
                                                onclick='requestUpdate("<%=c.getCategoryID()%>")'><i class="fas fa-edit"></i>
                                        </button>
                                    </td>
                                </tr>
                                <%}%>
        ...
        function confirmDelete(categoryID) {
            jQuery(function () {
                jQuery(".trash").append(function () {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn xóa danh mục này không?",
                        buttons: ["Hủy bỏ", "Đồng ý"]
                    })
                            .then((willDelete) => {
                                if (willDelete) {
                                    window.location.href = '/Project/CategoryServlet?service=delete&categoryID=' + categoryID;
                                }
                            });
                });
            });
    ...
    <script>
        function requestUpdate(categoryID) {
            window.location.href = '/Project/CategoryServlet?service=sendUpdateCategory&status=admin&categoryID=' + categoryID;
        }
    </script>
</body>
